...
def getWordScore(word, n):
    ...
    letter_score = []
    for i in word:
        letter_score.append(SCRABBLE_LETTER_VALUES[i])
    letter_sum = sum(letter_score)
    if l:
        if l == n:
            return letter_sum * l + 50
        else:
            return letter_sum * l
    else:
        return 0
        ...
        if user_word == ".":
         ...
         # Otherwise (the input is not a single period):
            ...
            if not isValidWord(user_word,hand,wordList):
                ...
                localscore = getWordScore(user_word, n)
                score += localscore
                print(" \"" + user_word + " \" earned " + str(localscore) + " points. Total: " + str(score) + " points.")
                # Update the hand
...
def playGame(wordList):
    ...
    while True:
        ...
        if user_input == 'e':
            ...
            last_hand = dealHand(HAND_SIZE).copy()
